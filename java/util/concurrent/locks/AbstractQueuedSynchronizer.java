...
public abstract class AbstractQueuedSynchronizer
    ...
    private Node enq(final Node node) {
        for (;;) {
            Node t = tail;
            // 队列为空，创建head节点，tail指向head
            if (t == null) { // Must initialize
                // 创建头结点
                // waitStatus = 0
                // 空构造方法，也就是说这个节点不属于任何线程 node.thread==null
     ...
     *
     * 返回true，表示前驱节点的ws=-1，是正常情况，当前节点线程需要被挂起，等待前驱节点拿到锁后，在释放锁的时候被唤起
     * 返回false，说明当前节点的线程不需要被挂起
     *
     */
    private static boolean shouldParkAfterFailedAcquire(Node pred, Node node) {
        ...
        // 前驱节点的waitStatus大于0，说明前驱节点取消了排队，向前遍历，直到找到一个ws小于0的前驱节点
        if (ws > 0) {
            /*
            ...
            do {
               ...
               // 一直遍历找前驱节点----找个好爹，让唤起当前线程====前驱节点的ws依赖后继节点
                node.prev = pred = pred.prev;
            } while (pred.waitStatus > 0);
            // 构造双向链表
            pred.next = node;
        ...
        // 返回FALSE后，继续执行while循环，走到ws=-1分支，返回true
        return false;
    }
    ...
    public final void acquire(int arg) {
        ...
        if (!tryAcquire(arg) &&
                // 尝试获取锁失败，当前线程挂起，入队
                // addWaiter----创建节点，并入队列
                // acquireQueued方法返回true，会执行下边的逻辑；正常情况下应该返回FALSE
    ...
    final int fullyRelease(Node node) {
        boolean failed = true;
        try {
            // 获取当前state值，可能是重入锁，所以state可能大于1
            int savedState = getState();
            // release，完全释放锁，state==0
            if (release(savedState)) {
                failed = false;
                // 返回释放前的state值
                return savedState;
            } else {
                // 这里也说明了执行await方法的前提是持有锁，否则release失败FALSE，直接抛出异常
    ...
    public class ConditionObject implements Condition, java.io.Serializable {
        ...
        private void unlinkCancelledWaiters() {
            // 单向链表遍历
            Node t = firstWaiter;
            // 一直指向遍历后的、最后一个ws正常的节点，连接下一个正常节点使用
            Node trail = null;
            while (t != null) {
                // 先找到下一个节点
        ...
        public final void await() throws InterruptedException {
            ...
            // 线程执行await方法前，必须是持有锁的，这里释放锁
            // 完全释放锁，将state减到0，并且返回释放前的state值savedState
            // 为什么需要返回？====被唤醒时需要重新持有savedState把锁
            int savedState = fullyRelease(node);

            /**
             * REINTERRUPT: await后需要重新弄设置中断标识
             * THROW_IE: await后需要抛出interruptedEx异常
             * 0: 正常，await期间没有中断发生
             */
            int interruptMode = 0;
            // 节点不在阻塞队列中
